...
//					return errorFound;
        ...
        if (theRule.ruleType != DRCTemplate.DRCRuleType.SURROUND)
        {
            ...
            /*
             * special case: ignore errors between the unconnected layer of a resistor node.
             */
            Geometric[] geoms = new Geometric[]{geom1, geom2};
            int[] nets = new int[]{net1, net2};
            for(int i=0; i<2; i++)
            {
            	if (!(geoms[i] instanceof NodeInst)) continue;
            	NodeInst ni = (NodeInst)geoms[i];
            	if (ni.getFunction() != PrimitiveNode.Function.RESIST) continue;
            	if (nets[i] < 0) return errorFound;
            }
